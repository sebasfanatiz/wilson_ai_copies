...
EXPANDABLE_FIELDS = {"primary_texts"}
...
def _polish_ending_es(t: str) -> str:
    ...
    s = t.strip()
    s = re.sub(r"[:—\-–]\s*$", "", s)
    s = re.sub(r"\s+(?:vs|VS)\s*$", "", s)
    s = re.sub(r"\s+(?:o|y)\s+on\s*$", "", s)
    s = re.sub(r"\s+on\s*$", "", s)
    s = re.sub(r"\s+(?:en|de|del)\s*$", "", s)
    tokens = s.split()
...
def _smart_trim(text: str, limit: int, lang: str = "es") -> str:
    ...
    out = []
    for w in t.split():
        if len((" ".join(out + [w])).strip()) <= limit: out.append(w)
        else: break
    s = " ".join(out).strip()
...
def _format_discount(val) -> str:
    ...
    try:
        f = float(s.replace(",", "."))
        if 0 < f < 1: return f"{int(round(f * 100))}%"
    except: pass
    if re.fullmatch(r"\d{1,3}", s): return f"{s}%"
...
def _sanitize_google_no_exclaim(struct_market: dict) -> dict:
    for camp in ['SEM', 'GoogleDemandGen', 'GooglePMAX']:
        if camp not in struct_market: continue
...
def _multiline_by_sentence(s: str) -> str:
    ...
    lines = []
    for p in parts:
        if len(p) <= 2 and p.lower() in STOPWORDS_END_ES: continue
        lines.append(p)
    return "\n".join(lines)
...
def _format_meta_primary_texts_multiline(struct_market: dict) -> dict:
    for camp in ['MetaDemandGen', 'MetaDemandCapture']:
        if camp not in struct_market: continue
        if 'primary_texts' not in struct_market[camp]: continue
...
def get_default_markets_for_platform(df_plans: pd.DataFrame, platform: str) -> list[str]:
    ...
    if not markets_set: return ['US/CA', 'EUROPE', 'ROW']
...
def obtener_info_contenido(campaign_name, brief, content_df, plans_df, platform, league_or_other):
    ...
    if df.empty and league_norm and league_norm != "otro":
        aliases = ["lvbp", "liga venezolana de beisbol profesional", "liga venezolana de béisbol profesional"]
        df2 = content_df.copy()
        df2["__name_norm"] = df2["content_name"].fillna("").astype(str).apply(_norm_str)
        mask_alias = False
        for a in aliases:
            mask_alias = mask_alias | df2["__name_norm"].str.contains(re.escape(_norm_str(a)))
        if mask_alias is not False: df = df2[mask_alias]
...
def chat_create(messages, model=None, max_retries=3, timeout=300, **kwargs):
    ...
    while attempt <= max_retries:
        try:
            return client.chat.completions.create(model=model, messages=messages, timeout=timeout, **kwargs)
...
def _expand_batch(texts: list, min_chars: int, max_chars: int, lang: str = 'es') -> tuple:
    ...
    resp = chat_create(model=MODEL_CHAT, messages=[{"role": "system", "content": sys_msg}, {"role": "user", "content": f"{prompt_inst}\n\n{bloques}"}])
    ...
    out = list(texts)
    for i, new in zip(idxs, cleaned): out[i] = _smart_trim(new, max_chars)
    return out, usage
...
def preparar_batch(texts: list, limit: int, tipo: str, lang: str = 'es') -> tuple:
    ...
    if idxs_long:
        ...
        for i, new_text in zip(idxs_long, cleaned): df.at[i, "Reescrito"] = _smart_trim(new_text, limit)

    min_chars = MIN_CHARS_BY_FIELD.get(tipo)
    if (tipo in MIN_CHARS_BY_FIELD) and isinstance(min_chars, int) and min_chars > 0:
...
def traducir_batch(texts: list, target_lang: str) -> tuple:
    if not texts or all(not t for t in texts) or target_lang not in ("en", "pt"): return texts, {}
...
def _ejemplos_referencia(ref_df: pd.DataFrame, n=5):
    ...
    if campo_col:
        ...
        rest_sample = df_rest.sample(n=min(n - len(primary_sample), len(df_rest)), random_state=43) if n - len(primary_sample) > 0 else pd.DataFrame()
    ...
    return "\n".join(f"- [{pick(r,'market','Market')}][{pick(r,'idioma','Idioma')}] {pick(r,'platform','Platform')} {pick(r,'tipo','Tipo')} {pick(r,'campo','Campo')}: \"{pick(r,'texto','Texto')}\"" for _, r in ejemplos.iterrows())
...
def _rules_block(plan_info_by_market: dict, base_lang: str, include_prices: bool = True) -> str:
    lines = [f"Idioma base de redacción: {base_lang.upper()}"]
    
    if not include_prices:
        lines.append("REGLA ESTRICTA: ESTÁ TOTALMENTE PROHIBIDO mencionar precios, planes, descuentos o valores monetarios. Enfócate ÚNICAMENTE en posicionamiento de marca, los equipos, la pasión y los beneficios de la plataforma.")
        return "\n".join(lines)
...
def _meta_rules_text(include_prices: bool = True):
    ...
    if include_prices:
        base += (
            "- Demand Capture: mencionar explícitamente el plan ANUAL + promo (si existe) con símbolo exacto.\n"
            "- NO inventar precios.\n"
        )
    return base

def prompt_google_pmax(briefs, ref_df, plan_info_by_market, base_lang='es', include_prices=True):
    ejemplos = _ejemplos_referencia(ref_df, n=5)
    schema = _google_pmax_schema_for_market(briefs['market'])
    rules = _rules_block(plan_info_by_market, base_lang, include_prices) + "\n" + _google_rules_text(include_prices)

    return f"""Eres un generador experto de copies para Google Ads.
...
- Moneda: usar EXACTAMENTE el símbolo indicado en los planes (Si aplica).
- Escribe TODO en Español (ES) sin mezclar idiomas.
""".strip()

def prompt_meta_all(briefs, ref_df, plan_info_by_market, base_lang='es', include_prices=True):
    ejemplos = _ejemplos_referencia(ref_df, n=5)
    template = {
        briefs['market']: {
            "MetaDemandGen": {
    ...
    rules = _rules_block(plan_info_by_market, base_lang, include_prices) + "\n" + _meta_rules_text(include_prices)
    return f"""Eres un generador experto de copies para Meta Ads.
...
def _ensure_list_of_len(value, length):
    ...
    if len(arr) < length: arr += [""] * (length - len(arr))
    return arr[:length]
...
def _post_enforce_counts_and_limits(struct: dict):
    for market, cmap in struct.items():
        for campaign, fields in CAMPAIGNS_STRUCTURE.items():
            if campaign not in cmap: continue
...
def _lowest_price_token(plans: list[dict]) -> str | None:
    ...
    for p in plans or []:
        try:
            ...
            if (best is None) or (price < best[0]): best = (price, token)
        except: continue
...
def _annual_plan_with_promo(plans: list[dict]) -> dict | None:
    for p in plans or []:
        ...
        if 'annual' in rp or 'anual' in rp:
            if p.get('marketing_discount'): return p
    return None
...
def _replicate_google_from_pmax(google_struct_for_market: dict):
    ...
    for camp in ['SEM', 'GoogleDemandGen', 'GooglePMAX']:
        if camp not in google_struct_for_market[market]: continue
        ...
        for campaign, fields in CAMPAIGNS_STRUCTURE.items():
            ...
            for field, (count, limit) in fields.items():
                ...
                if any(_seems_english(t) for t in es_texts):
                    ...
                    for k in total_usage: total_usage[k] += getattr(usage_fix, k, 0) or 0
                for k in total_usage: total_usage[k] += getattr(usage, k, 0) or 0
...
def generar_copies(
    ...
    include_prices: bool = True,  # NUEVA BANDERA
) -> tuple:
    print(f"Iniciando generación para '{campaign_name}' | Plataforma: {platform_name} | Liga: {league_selection} | Langs: {langs_csv} | Precios: {include_prices}")
    ...
    # REGLA DE MARKETS: Si NO queremos precios, forzamos un bloque único llamado "Global"
    if not include_prices:
        markets_to_process = ["Global"]
    else:
        ...
        if not markets_to_process:
            return output_filename, "Error: no se encontraron mercados válidos para procesar."
    ...
    for market in markets_to_process:
        ...
        if include_prices:
            ...
            if not valid_plans:
                print(f"  ⚠️ No se encontraron planes válidos en '{market}'. Saltando...")
                continue
        ...
        google_data_market = {}
        try:
            prompt_g = prompt_google_pmax(briefs, df_refs, {market: plan_info_market}, base_lang='es', include_prices=include_prices)
            resp_g = chat_create(
                model=MODEL_CHAT, response_format={"type": "json_object"},
                messages=[
        ...
        meta_data_market = {}
        try:
            prompt_m = prompt_meta_all(briefs, df_refs, {market: plan_info_market}, base_lang='es', include_prices=include_prices)
            resp_m = chat_create(
                model=MODEL_CHAT, response_format={"type": "json_object"},
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that outputs STRICT JSON only."},
        ...
        # Solo aplicamos inyección semántica de precios si está activa
        if include_prices:
